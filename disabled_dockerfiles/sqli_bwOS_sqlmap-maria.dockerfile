...
ENV APP_NAME "SQLmap"
# Note: Removed the single quotes around the tamper script names
ENV CMDLINE_EXEC 'python3 sqlmap.py -p a --tmp-dir=/tmp --no-logging --flush-session --batch --random-agent --url ${TARGET_URL} --level 5 --risk 3 --all --dbms=MariaDB'
