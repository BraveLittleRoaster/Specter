...
ENV APP_NAME "SQLmap"
ENV CMDLINE_EXEC 'python3 sqlmap.py -p a --tmp-dir=/tmp --no-logging --flush-session --batch --random-agent --url ${TARGET_URL} --data=${POST_BODY} -p ${INJECTION_POINT} --level 5 --risk 3 --all'
...
CMD ["/bin/bash", "-c", "export TARGET_URL=\"${TARGET_URL/?a=1}\" && exec /usr/src/app/start.sh"]
